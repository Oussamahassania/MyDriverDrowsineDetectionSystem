...
while True:
    ...
    # Ensure grayscale image is 8-bit and valid
    if gray.dtype != "uint8":
        gray = cv2.convertScaleAbs(gray)

    print(f"[DEBUG] Gray image dtype: {gray.dtype}, shape: {gray.shape}")

    # Ensure the image is in the correct format for dlib
    # This ensures the image passed to dlib is correctly formatted
    if gray.shape[2] != 1:  # Check if the image is not grayscale
    ...
    if len(rects) > 0:
        for rect in rects:
            ...
            w = int(shape[:, 0].max() - shape[:, 0].min()) + 20
            h = int(shape[:, 1].max() - shape[:, 1].min()) + 20
            ...
            try:
                ...
                cv2.putText(display_frame, label, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
